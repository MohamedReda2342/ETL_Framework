...
def edit_user_dialog(username):
    ...
    with st.form(f"edit_user_form_{username}"):
        ...
        with col2:
            ...
            edit_roles = st.multiselect("Roles", ["user", "admin"], default=user_data['roles'])
...
def add_user_dialog():
    ...
    with st.form("add_user_form"):
        ...
        if add_submitted:
            if new_username and new_first_name and new_last_name and new_password and new_roles:
...
def edit_role_dialog(role_name):
    ...
    with st.form(f"edit_role_form_{role_name}"):
        edit_description = st.text_area("Description", value=role_data.get('description', ''))       
        edit_privileges = st.multiselect(
            "Privileges", 
            privileges, 
            default=role_data.get('privileges', [])
        )
...
def delete_role_dialog(role_name):
    ...
    users_with_role = [username for username, user_data in users.items() 
                      if role_name in user_data.get('roles', [])]
    
    if users_with_role:
        st.error(f"❌ Cannot delete role '{role_name}' because it's assigned to the following users:")
...
with tab2:    
    ...
    for role_name, role_data in roles.items():
        ...
        with col4:
            ...
            if st.button("Delete", key=f"delete_role_button_{role_name}", 
    ...
    with st.form(f"edit_privilege_form_{old_privilege}"):
        ...
        if update_submitted:
            if new_privilege_name:
                if new_privilege_name != old_privilege and new_privilege_name in privileges:
                    ...
                    for role_name, role_data in roles.items():
                        if old_privilege in role_data.get('privileges', []):
                            ...
                            role_privileges[role_privileges.index(old_privilege)] = new_privilege_name
                    
                    save_config(config)
                    st.success(f"✅ Privilege updated from '{old_privilege}' to '{new_privilege_name}' successfully!")
                    st.rerun()
                else:
...
def delete_privilege_dialog(privilege_name):
    ...
    roles_with_privilege = [role_name for role_name, role_data in roles.items() 
                           if privilege_name in role_data.get('privileges', [])]
    
    if roles_with_privilege:
        st.error(f"❌ Cannot delete privilege '{privilege_name}' because it's assigned to the following roles:")
    ...
    # Confirmation checkbox
    confirm_delete = st.checkbox(f"I confirm I want to delete privilege '{privilege_name}'")
...
with tab3:
    ...
    for privilege in privileges:
        ...
        with col2:
            if st.button("Edit", key=f"edit_privilege_button_{privilege}", type="primary"):
                edit_privilege_dialog(privilege)
        
        with col3:            
            if st.button("Delete", key=f"delete_privilege_button_{privilege}", help=help_text):
