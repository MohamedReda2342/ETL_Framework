...
if uploaded_file is not None:
    ...
    # Key Domain Name 
    with multi_col1:
        key_domain_names = df_utlis.get_unique_values(filtered_key_set_names_DF, 'Key Domain Name')
...
#---------------------------------------    Display query editor and execute query    ---------------------------------------
    ...
    with exec_query_col:
        if st.button(f"Execute Query", key=f"Execute_Query_Bttn"):
            ...
            query_to_execute = st.session_state.get(f"query_editor", "") 
            if query_to_execute:
                st.success(f"Executed: {query_to_execute}")
            else:
                st.warning("Query editor is empty. Nothing to execute.")
#------------------------------------------------------------------------------------------------------------------------------
