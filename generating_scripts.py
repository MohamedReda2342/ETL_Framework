...
'''
def get_env_value(row, env_dict):
    print(row)
    if row['source']=='env':
        row['parameter']=env_dict[row['parameter_name']]
    
    return row
'''

def define_script(df):
    script_string=''
...
def get_process_name(Filtered_tables_DF):
    ...
    '''process_list = Filtered_tables_DF.apply(lambda row: row['operation']+" " + row['schema']+'.' + row['functions']+'('+row['function_parameters']+")", axis=1)
f"BK_{row['Key Set Name']}_{row['Key Domain Name']}_{row['Table Name Source']}_{row['Column Name Source']}"
    '''
    print(process_names)
...
def get_bkey_call_script(smx_model, filtered_functions, env , bigint_flag):
    print('================     get bkey call script =====================================================')
    #print(smx_model)
    print(filtered_functions)
    print(filtered_functions.columns)
    script=" place holder "
    df_by_function=filtered_functions.groupby(['operation', 'schema','functions' ])
    print(df_by_function)
    print(type(df_by_function))
    env_attributes=get_env_dict(env, bigint_flag)
    scripts=[]
    print("============ staging model ", type(smx_model))
    print(type(smx_model['stg tables']))
    for group_name, df_group in df_by_function:
        print("\n-- Group with {} rows(s)".format(len(df_group)))
        print('{}'.format(group_name))
        print(df_group)
        print('==================== apply schema as per the environment ==========', env)
        ...
        df_group['parameter'] = df_group.apply(lambda row: str(env_attributes[row.parameter_name]) if  row.source=='env' else 'SMX', axis=1)
        print("====================== printing df_group.columns")

        print(df_group.columns)
        print(df_group[['parameter','source']])
        ...
        parameters_string = tuple(df_group['parameter'])
        s= ",".join([str(s) for s in parameters_string])
        print("====================== printing paramter_string")
        print(parameters_string)
        print("====================== s")

        print(s)
        df['function_parameters']=s
        print(df)

        df=df.drop_duplicates()
        print(df)
        #u= np.unique(df[['operation', 'schema', 'functions', 'function_parameters']].values)
        #print(u)
        print(df.shape)
        ...
        print(df)
        print(df[['source','smx_columns']])
        smx_df=smx_model['stg tables']
        clean_df = smx_df.loc[smx_df['key set name'].notnull()]
        clean_df = clean_df.loc[clean_df['key domain name'].notnull()]


        process_names= get_process_name(clean_df)
        print(process_names)
        #print(smx_model['stg tables'].columns)
        df2=pd.DataFrame()
        df2['process_name']=process_names
        print(df2)
        
        df['script'] = df.apply(lambda row: row['operation']+" " + row['schema']+'.' + row['functions']+'('+row['function_parameters']+")", axis=1)
        print("=====================================    printing script ===================================================")
        print(df['script'])

        df_merge = pd.merge(df, df2, left_index=True, right_index=True, how = 'outer')
        df_merge = df_merge.ffill()
        print(df_merge)
        #print(df_merge)
        df_merge['script'] = df_merge.apply(lambda x: x['script'].replace("SMX", str(x['process_name'])), axis=1)

        #print(df)
        #print(df_merge)
        scripts.append(df_merge['script'])

    

    return scripts

def get_smx_values(smx_tab, smx_list,smx_dict):
    print(Back.LIGHTRED_EX+ "======================     get_smx_values          ===================================")
    print(smx_list)
    print(type(smx_list))
    print(len(smx_list))
    print(Back.GREEN + smx_tab)
    #l= smx_list.split(",")
    l=[s.strip().lower() for s in smx_list]
    print(Back.LIGHTMAGENTA_EX + str(l))
    ...
    print(smx_dict.keys())
    #smx_tab= smx_list[0]
    #ll =  [ ast.literal_eval(x) for x in smx_list]
    #print(ll)
    #print(smx_tab)
    smx_df = smx_dict[smx_tab.lower()]
    print(smx_df)
    print(Back.RED+ smx_df.columns)
    smx_df=smx_df[l]
    print(smx_df)
...
def get_params_values(smx_tab, df, smx_dict):
    ...
    multiple_source_list = [x for x in smx_source_list if len(x) > 0] #[ len(l) for l in smx_source_list]
...
def get_bkey_reg_script(smx_model, filtered_script_df, env_attributes):
    ...
    print("==========================================================================")
    #filtered_script_df=filtered_script_df[['operation', 'schema', 'functions', 'env_schema',
     #  'parameters', 'parameter_name', 'source', 'smx_column', 'presentation_col']]
    print(filtered_script_df)
    df_by_function=filtered_script_df.groupby(['operation', 'schema','functions' ], sort=False)
    # for loop to show the group of function
    #for key, group in df_by_function:
    #   print(Back.RED + f"Group Name: {key}")
     #  print(Back.GREEN + group)
    ...
    for group_name, df_group in df_by_function:
        ...
        smx_tab= df_group['source'].unique()
        print(df_group.columns)
        df_group['parameter'] = df_group.apply(lambda row: f'"{str(env_attributes[row.parameter_name])}"' if  row.source=='env' else row.smx_column, axis=1)
        print(df_group)
        ...
        #params_df = get_smx_values(smx_tab[0],list(df['function_parameters'])[0], smx_model)
        source_params_df= df[['source', 'parameter', 'smx_column','parameters', 'presentation_col','join_key']]
...
def main(smx_model, key_type, env , bigint_flag):
   ...
   # print(type(smx_df))
    ...
    script = None  # Initialize script variable
    ...
    match key_type:
        ...
        case "REG_BKEY_PROCESS": #System".lower():
            print("key type = ", key_type)
            print(filtered_script_df.columns)
            script= get_bkey_domain_script(smx_model, filtered_script_df, env)
            print(script)
